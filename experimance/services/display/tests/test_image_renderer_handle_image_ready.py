#!/usr/bin/env python3
"""
Tests for ImageRenderer.handle_image_ready method.
...
class TestImageRendererHandleImageReady:
    ...
    @pytest.mark.asyncio
    async def test_handle_image_ready_with_file_uri(self):
        ...
        try:
            ...
            with patch('experimance_display.renderers.image_renderer.pyglet.image.load', return_value=mock_pyglet_image), \
                ...
                await self.renderer.handle_image_ready(message)
    ...
    @pytest.mark.asyncio
    async def test_handle_image_ready_with_base64(self):
        """Test handle_image_ready with base64 message."""
    ...
    @pytest.mark.asyncio
    async def test_handle_image_ready_with_cached_image(self):
        """Test handle_image_ready when image is already cached."""
        # Pre-populate cache
        mock_image = Mock()
        mock_sprite = Mock()
    ...
    @pytest.mark.asyncio
    async def test_handle_image_ready_with_missing_image_id(self):
        """Test handle_image_ready with malformed message."""
    ...
    @pytest.mark.asyncio  
    async def test_handle_image_ready_with_load_failure(self):
        """Test handle_image_ready when image loading fails."""
        message = {
    ...
    @pytest.mark.asyncio
    async def test_handle_image_ready_temp_file_cleanup(self):
    ...
    @pytest.mark.asyncio
    async def test_handle_image_ready_with_hybrid_transport(self):
        """Test handle_image_ready with hybrid transport message."""
        # Save test image to temporary file
        with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as temp_file:
            self.test_image.save(temp_file.name)
            temp_path = temp_file.name
    ...
    @pytest.mark.asyncio
    async def test_handle_image_ready_exception_handling(self):
        """Test that exceptions in handle_image_ready are properly caught and logged."""
