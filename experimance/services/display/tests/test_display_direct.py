...
def test_config():
    ...
    return DisplayServiceConfig(
        ...
        display=DisplayConfig(
            ...
            resolution=(800, 600),
            debug_overlay=True,
            headless=True  # Enable headless mode for testing
        )
    )
...
async def test_direct_interface_sequence(test_config, mock_pyglet, mock_zmq_subscriber):
    ...
    async with active_service(service) as active:
        ...
        active._direct_handlers["image_ready"] = handle_image_ready_mock
        active._direct_handlers["text_overlay"] = handle_text_overlay_mock
        active._direct_handlers["video_mask"] = handle_video_mask_mock
        
        # Test text overlay
        active.trigger_display_update("text_overlay", {
            "text_id": "test_text_1",
        ...
        active.trigger_display_update("image_ready", {
