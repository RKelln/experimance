...
class DisplayCLI:
    ...
    async def send_remove_text(self, text_id: str):
        ...
        message = {"type": MessageType.REMOVE_TEXT, "text_id": text_id}

        await self.pubsub_service.publish(message, topic=MessageType.REMOVE_TEXT)
        logger.info(f"Sent RemoveText: {text_id}")

    async def send_video_mask(
        self, mask_path: str, fade_in_duration: float = 0.2, fade_out_duration: float = 1.0
    ):
    ...
    async def send_era_changed(self, era: str, biome: str):
        ...
        message = {"type": MessageType.SPACE_TIME_UPDATE, "era": era, "biome": biome}
    ...
    async def send_loop_ready(
        self, loop_path: str, still_uri: str, loop_type: str = "idle_animation"
    ):
...
def generate_panorama_tile_image(
    width: int, height: int, tile_id: str, position: tuple = (0, 0)
) -> str:
...
async def send_display_image(
    cli: "DisplayCLI", image_path: str | None, x: Optional[int] = None, y: Optional[int] = None
):
...
async def test_panorama(args, cli):
    ...
    while True:
        try:
            ...
            base_image_path = generate_panorama_base_image(
                args.base_width, args.base_height, "PANORAMA_BASE"
            )
            ...
            for i in range(args.tile_count):
                ...
                tile_path = generate_panorama_tile_image(
                    args.tile_width, args.tile_height, f"{i + 1}", (x_pos, y_pos)
                )
                ...
                print(
                    f"  (Generated as {args.tile_width}x{args.tile_height}, will be scaled to {target_tile_width}x{target_tile_height})"
                )
...
async def test_scaling(args, cli):
    ...
    for pano_width, pano_height, description in test_dimensions:
        ...
        # Generate test image
        test_image_path = generate_panorama_base_image(
            pano_width,
            pano_height,
...
async def cycle_images_command(
    cli: DisplayCLI, directory: Optional[str] = None, interval: float = 3.0
):
...
async def demo_command(cli: DisplayCLI):
    ...
    await cli.send_text_overlay(
        "system_info", "System Status: Running", "system", None, "top_right"
    )
    await asyncio.sleep(2)
...
def main():
    ...
    text_parser.add_argument(
        "content", nargs="?", help="Text content to display (random if not provided)"
    )
    ...
    # Marquee command
    marquee_parser = subparsers.add_parser("marquee", help="Send circular marquee text overlay")
    marquee_parser.add_argument(
        "content",
        nargs="?",
        help="Text content to display around the circle (random if not provided)",
    )
    marquee_parser.add_argument(
        "--id", default=None, help="Text ID (auto-generated if not provided)"
    )
    marquee_parser.add_argument(
        "--duration", type=float, default=None, help="Duration in seconds (None = infinite)"
    )
    marquee_parser.add_argument("--font-size", type=int, default=None, help="Font size in pixels")
    marquee_parser.add_argument("--font-name", default=None, help="Font family name")
    marquee_parser.add_argument(
        "--write-speed", type=float, default=None, help="Characters written per second"
    )
    marquee_parser.add_argument(
        "--no-loop", action="store_true", help="Do not loop text; stop after one pass"
    )
    marquee_parser.add_argument(
        "--no-follow",
        action="store_true",
        help="Ignore previous write-head position; always start from --start-angle (default: follow)",
    )
    marquee_parser.add_argument(
        "--gap-slots", type=int, default=None, help="Gap slot count ahead of write head"
    )
    marquee_parser.add_argument(
        "--radius-scale",
        type=float,
        default=None,
        help="Radius scale relative to mask circle (default 0.97 — just inside mask edge)",
    )
    marquee_parser.add_argument(
        "--char-spacing",
        type=float,
        default=None,
        help="Slot width multiplier: >1.0 spreads chars apart, <1.0 packs tighter (default 1.0)",
    )
    marquee_parser.add_argument(
        "--start-angle",
        type=float,
        default=None,
        help="Start position in degrees clockwise from 12 o'clock (default 0 = top)",
    )
    ...
    mask_parser.add_argument(
        "path", nargs="?", help="Path to mask image file (default mock if not provided)"
    )
    ...
    trans_parser.add_argument(
        "path", nargs="?", help="Path to transition video/image (default if not provided)"
    )
    ...
    loop_parser.add_argument(
        "still_uri",
        nargs="?",
        help="URI of still image this loop animates (random if not provided)",
    )
    ...
    cycle_parser.add_argument(
        "directory", nargs="?", help="Directory with images (uses default if not provided)"
    )
    cycle_parser.add_argument(
        "--interval", type=float, default=3.0, help="Interval between images in seconds"
    )
    ...
    panorama_parser.add_argument(
        "--config", type=str, help="Display service config file (required for panorama mode)"
    )
    panorama_parser.add_argument(
        "--base-width",
        type=int,
        default=5760,
        help="Base image width (default: 11520 - includes mirroring)",
    )
    panorama_parser.add_argument(
        "--base-height", type=int, default=1080, help="Base image height (default: 1080)"
    )
    panorama_parser.add_argument(
        "--tile-count", type=int, default=3, help="Number of tiles to generate (default: 3)"
    )
    panorama_parser.add_argument(
        "--tile-width",
        type=int,
        default=800,
        help="Generated tile image width (default: 800) - positioning uses config",
    )
    panorama_parser.add_argument(
        "--tile-height",
        type=int,
        default=600,
        help="Generated tile image height (default: 600) - positioning uses config",
    )
    ...
    scaling_parser.add_argument(
        "--mode",
        choices=["width", "height", "shortest"],
        default="width",
        help="Rescale mode to test (default: width)",
    )
    scaling_parser.add_argument(
        "--show-info", action="store_true", help="Show detailed scaling calculations"
    )

    # Stress test command
    stress_parser = subparsers.add_parser(
        "stress", help="Run stress test (randomly send text, image, or change map)"
    )
    stress_parser.add_argument(
        "interval", type=float, help="Interval in seconds between sends (float)"
    )
    ...
    async def run_command():
        ...
        try:
            ...
            if args.command == "image":
                ...
                await cli.send_text_overlay(
                    text_id, content, args.speaker, args.duration, args.position
                )
                ...
                if args.write_speed is not None:
                    style["write_speed"] = args.write_speed
                if args.gap_slots is not None:
                    style["gap_slots"] = args.gap_slots
                if args.radius_scale is not None:
                    style["radius_scale"] = args.radius_scale
                if args.char_spacing is not None:
                    style["char_spacing"] = args.char_spacing
                if args.start_angle is not None:
                    style["start_angle"] = args.start_angle
                if args.no_loop:
                    style["loop"] = False
                if args.no_follow:
                    style["follow"] = False
                ...
                await cli.pubsub_service.publish(message, topic=MessageType.DISPLAY_TEXT)
                logger.info(f"Sent marquee text: {text_id!r} — '{content[:60]}'")
                ...
                if args.era and args.biome:
                    ...
                    biome = (
                        random.choice(matching_biomes)
                        if matching_biomes
                        else random.choice([b for _, b in DEFAULT_ERAS_BIOMES])
                    )
                else:
                ...
                still_uri = (
                    args.still_uri or f"file://{get_random_image()}"
                    if get_random_image()
                    else "file:///tmp/placeholder.png"
                )
                if not loop_path:
                ...
                print("\nMock Files:")
                for mock_file in [
                    MOCK_IMAGES_DIR_ABS / "mock_depth_map.png",
                    ...
                    if action == "text":
                        ...
                        if random.random() < 0.8:
                            ...
                            if len(active_text_ids) >= 10:
                                ...
                                logger.debug(
                                    f"Stress: Removed text overlay {text_id}, remaining: {len(active_text_ids)}"
                                )
                            content = random.choice(DEFAULT_TEXTS)
                            text_id = str(uuid.uuid4())[:8]
                            active_text_ids.add(text_id)  # Track this text ID
                            ...
                            duration = random.choice([None, 2.0, 5.0, 10.0])
                            await cli.send_text_overlay(
                                text_id, content, speaker, duration, position
                            )
                            ...
                            run_command.progressive_texts[text_id] = {
                                "words": content.split(),
                                "idx": 1,
                                "speaker": speaker,
                                "position": position,
