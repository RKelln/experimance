...
from .renderers.marquee_text_renderer import MarqueeTextRenderer
...
class DisplayService(BaseService):
    ...
    def __init__(self, config: DisplayServiceConfig):
        ...
        self.marquee_text_renderer = None
    ...
    async def start(self):
        ...
        if self.config.zmq.subscriber:
            logger.info(
                f"✅ ZMQ service started, subscriber listening on {self.config.zmq.subscriber.address}:{self.config.zmq.subscriber.port}"
            )
        ...
        logger.info(
            f"DisplayService started on {self.window.width}x{self.window.height}"
            if self.window
            else "DisplayService started (no window)"
        )
    ...
    def _initialize_window(self):
        ...
        try:
            ...
            forced_headless = os.environ.get("EXPERIMANCE_DISPLAY_HEADLESS", "").lower() in (
                "true",
                "1",
                "yes",
            )
            ...
            if forced_headless:
                logger.info(
                    "Headless mode forced via EXPERIMANCE_DISPLAY_HEADLESS environment variable"
                )
            elif headless:
            ...
            if self.config.display.fullscreen:
                ...
                if self.config.display.monitor >= len(screens):
                    raise ValueError(
                        f"Invalid monitor index {self.config.display.monitor} for available screens: {len(screens)}"
                    )
                screen = screens[self.config.display.monitor]
                self.window = pyglet.window.Window(
                    fullscreen=True, screen=screen, vsync=self.config.display.vsync
                )
            else:
                # Windowed mode
            ...
            logger.info(
                f"Window initialized: {self.window.width}x{self.window.height}, fullscreen={self.config.display.fullscreen}"
            )
        ...
        try:
            ...
            if self.config.panorama.enabled:
                ...
                self.image_renderer = ImageRenderer(
                    ...
                    ),  # Increment layer count for each renderer,
                )
            ...
            self.marquee_text_renderer = MarqueeTextRenderer(
                config=self.config,
                window=self.window,
                batch=batch,
                order=(layer_count := layer_count + 1),
            )
            self.layer_manager.register_renderer("marquee_text", self.marquee_text_renderer)
    ...
    def _initialize_shader_effects(self, batch, layer_count):
        ...
        try:
            ...
            for effect_name, effect_config in self.config.shader_effects.effects.items():
                ...
                logger.info(
                    f"Registered shader effect: {effect_name} (order: {effect_config.order})"
                )

            logger.info(
                f"Shader effects initialization complete - {len([e for e in self.config.shader_effects.effects.values() if e.enabled])} effects loaded"
            )
        ...
        try:
            ...
            match message.get("content_type"):
                case ContentType.IMAGE:
                    logger.info(
                        f"📸 Routing IMAGE to renderer (panorama={bool(self.panorama_renderer)})"
                    )
    ...
    async def _handle_text_overlay(self, message: MessageDataType):
        ...
        try:
            ...
            # Route marquee speaker to the dedicated renderer
            if display_text.speaker == "marquee":
                if self.marquee_text_renderer:
                    await self.marquee_text_renderer.handle_text_overlay(display_text)
                else:
                    logger.warning("Marquee text renderer not initialised, dropping message")
                return
    ...
    async def _handle_remove_text(self, message: MessageDataType):
        ...
        try:
            ...
            # Forward to both text managers; each silently ignores unknown IDs
            if self.text_overlay_manager:
                await self.text_overlay_manager.handle_remove_text(message)
            if self.marquee_text_renderer:
                await self.marquee_text_renderer.handle_remove_text(message)
    ...
    def trigger_display_update(self, update_type: str, data: MessageDataType):
        ...
        if update_type in self._direct_handlers:
            ...
            clock.schedule_once(
                lambda dt: asyncio.create_task(self._direct_handlers[update_type](data)), 0
            )
    ...
    def _update_frame(self, dt):
        ...
        if self.fps_display_timer >= 1.0:
            ...
            if hasattr(self, "adaptive_stats") and self.adaptive_stats["processing_times"]:
                avg_proc = sum(self.adaptive_stats["processing_times"]) / len(
                    self.adaptive_stats["processing_times"]
                )
                avg_sleep = sum(self.adaptive_stats["sleep_times"]) / len(
                    self.adaptive_stats["sleep_times"]
                )
                target_frame_time = 1.0 / self.target_fps
                adaptive_info = f" - Adaptive: proc={avg_proc * 1000:.2f}ms, sleep={avg_sleep * 1000:.2f}ms, target={target_frame_time * 1000:.2f}ms"

            logger.info(
                f"FPS: {fps:.1f} (target: {self.target_fps}, headless: {self.config.display.headless}) - "
    ...
    def _on_draw(self):
        ...
        if self.config.display.profile and self._draw_count % 30 == 0:
            logger.debug(
                f"Draw timing - Total: {total_draw_time * 1000:.2f}ms, Render: {render_time * 1000:.2f}ms"
            )
    ...
    def _on_key_press(self, symbol, modifiers):
        ...
        if symbol == key.ESCAPE or symbol == key.Q:
            ...
            if self.panorama_renderer:
                ...
                logger.info(
                    f"Debug tiles toggle requested: current={current_state}, tiles_count={tile_count}"
                )
    ...
    async def _show_title_screen(self):
        ...
        try:
            ...
            logger.debug(
                f"Title screen will be visible for {self.config.title_screen.duration} seconds"
            )
...
async def run_display_service(
    config_path: str = "config.toml", args: Optional[argparse.Namespace] = None
) -> None:
    """
