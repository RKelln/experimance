...
This document covers the Pipecat backend implementation for the agent service. The backend provides local audio processing with speech-to-text, LLM conversation, and text-to-speech capabilities running in a single process.
...
On Linux, install audio development headers:
