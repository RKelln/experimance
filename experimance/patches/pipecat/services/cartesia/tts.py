...
class CartesiaTTSService(AudioContextWordTTSService):
    ...
    async def _disconnect(self, force: bool = False):
        ...
        Args:
            force: If True, skip graceful context cleanup for faster shutdown.
        """
        if not self._connected and not self._websocket:
            return

        try:
            # Cancel receive task first to prevent interference with cleanup
            if self._receive_task:
                try:
                    await self.cancel_task(self._receive_task)
                except Exception as e:
                    logger.debug(f"Error cancelling receive task: {e}")
                    # If cancel_task fails, try direct cancellation
            ...
            # For forced shutdowns, skip graceful context cleanup to speed up shutdown
            if not force and self._context_id and self._websocket and not self._websocket.closed:
                try:
                    # Attempt graceful context cleanup
                    cancel_msg = json.dumps({"context_id": self._context_id, "cancel": True})
                    await asyncio.wait_for(self._websocket.send(cancel_msg), timeout=0.5)
                except asyncio.TimeoutError:
                    logger.debug("Timed out sending context cancellation message")
                except websockets.exceptions.ConnectionClosed:
                    logger.debug("WebSocket already closed during context cleanup")
                except Exception as e:
                    logger.debug(f"Error during context cleanup: {e}")
            else:
                if force:
                    logger.debug("Forced shutdown: skipping graceful context cleanup")
    ...
    async def _handle_interruption(self, frame: StartInterruptionFrame, direction: FrameDirection):
        ...
        if self._context_id:
            try:
                ...
                logger.debug(f"Error sending cancellation during interruption: {e}")
            finally:
                self._context_id = None
    ...
    async def flush_audio(self):
        ...
        try:
            ...
            logger.debug(f"Error flushing audio: {e}")
        finally:
            self._context_id = None
    ...
    async def _receive_messages(self):
        try:
            ...
            logger.error(f"Error in receive messages: {e}")
        finally:
            self._connected = False

    async def _receive_task_handler(self, report_error_callback):
        """Handle the receive task with proper error reporting."""
        try:
            await self._receive_messages()
        except asyncio.CancelledError:
            logger.debug("Receive task handler cancelled")
            raise
        except Exception as e:
            logger.error(f"Error in receive task handler: {e}")
            if report_error_callback:
                await report_error_callback(e)
...
class CartesiaHttpTTSService(TTSService):
    ...
    async def cancel(self, frame: CancelFrame):
        ...
        try:
            ...
            logger.debug(f"Error closing HTTP client during cancel: {e}")
    ...
    async def run_tts(self, text: str) -> AsyncGenerator[Frame, None]:
        ...
        try:
            ...
            if self._settings["speed"]:
                request_data["speed"] = self._settings["speed"]

            response = await self._client.tts.bytes(
                **request_data
            )
