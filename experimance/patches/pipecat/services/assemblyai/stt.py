...
class AssemblyAISTTService(STTService):
    ...
    async def _disconnect(self, force: bool = False):
        ...
        Args:
            force: If True, skip graceful termination handshake for faster shutdown.
        """
        ...
        try:
            ...
            # For forced shutdowns, skip graceful termination to speed up shutdown
            if not force and not self._websocket.closed:
                try:
                    ...
                    # Wait for termination response with short timeout for graceful shutdowns
                    try:
                        await asyncio.wait_for(
                            self._termination_event.wait(),
                            timeout=1.0,  # Short timeout even for graceful shutdowns
                        )
                    except asyncio.TimeoutError:
                        logger.debug("Timed out waiting for termination message from server")

                except websockets.exceptions.ConnectionClosed:
                    logger.debug("WebSocket already closed during termination handshake")
                except Exception as e:
                    logger.debug(f"Error during termination handshake: {e}")
            else:
                if force:
                    logger.debug("Forced shutdown: skipping graceful termination handshake")
