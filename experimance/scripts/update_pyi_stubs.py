#!/usr/bin/env python3
"""
Regenerate .pyi type stub files for experimance_common.

Generates type stubs for dynamically loaded modules:
  - experimance_common/schemas.pyi
  - experimance_common/constants.pyi

Run after adding schemas or constants to any project or to the shared library.
...
See docs/update_pyi_stubs.md for details.
"""
...
def get_constants_base_exports() -> List[str]:
    ...
    constants_base_path = (
        PROJECT_ROOT / "libs" / "common" / "src" / "experimance_common" / "constants_base.py"
    )
...
def get_schemas_base_classes() -> List[str]:
    ...
    schemas_base_path = (
        PROJECT_ROOT / "libs" / "common" / "src" / "experimance_common" / "schemas_base.py"
    )
...
Examples:
  ...
  %(prog)s --dry-run --temp   # Save to temp files and show paths
    ...
    parser.add_argument(
        "--diff", action="store_true", help="Show diffs and ask for confirmation before writing"
    )
    parser.add_argument(
        "--temp", action="store_true", help="Save to temporary files (useful with --dry-run)"
    )
