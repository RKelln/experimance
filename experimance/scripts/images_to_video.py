...
Image filenames must follow: prefix_YYYYMMDD_HHMMSS_uuid.jpg

Usage:
    uv run scripts/images_to_video.py media/images/generated -o output.mp4
    uv run scripts/images_to_video.py media/images/generated -o output.mp4 \
        --start-time "2025-07-27 20:00:00" --end-time "2025-07-27 22:00:00"
    uv run scripts/images_to_video.py media/images/generated --list-only

See docs/images_to_video.md for full option reference.

Requirements: ffmpegcv, numpy, Pillow
"""
...
def create_video(
    ...
    try:
        ...
        out = ffmpegcv.VideoWriter(str(output_path), codec=codec, fps=fps, **vcodec_params)
...
Examples:
  ...
  %(prog)s ../../fmc/images --list-only
    ...
    parser.add_argument("image_dir", type=Path, help="Directory containing timestamped images")
    ...
    parser.add_argument(
        "--start-time", type=str, help='Start time filter (e.g., "2025-07-27 20:00:00")'
    )

    parser.add_argument(
        "--end-time", type=str, help='End time filter (e.g., "2025-07-27 22:00:00")'
    )
    ...
    parser.add_argument(
        "--fps", type=int, default=30, help="Frames per second for output video (default: 30)"
    )
    ...
    parser.add_argument(
        "--list-only", action="store_true", help="List images without creating video"
    )
