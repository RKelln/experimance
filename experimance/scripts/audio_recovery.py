...
Diagnoses and recovers USB audio devices: Yealink conference mic,
ICUSBAUDIO7D 5.1 card, ReSpeaker XVF3800 mic array.
...
Usage:
    ...
    uv run python scripts/audio_recovery.py reset-yealink
    uv run python scripts/audio_recovery.py test-respeaker
    uv run python scripts/audio_recovery.py fix-respeaker

See docs/audio_recovery.md for full command reference and device notes.
"""
...
from experimance_common.audio_utils import (
    list_audio_devices,
    reset_audio_device_by_name,
    cleanup_audio_resources,
    force_audio_system_reset,
    validate_audio_device_index,
)
...
def stop_audio_services() -> bool:
    ...
    try:
        ...
        try:
            ...
            subprocess.run(["jack_control", "stop"], capture_output=True, timeout=10)
            time.sleep(1)
            success = True
        ...
        try:
            ...
            subprocess.run(["pkill", "-f", "pipewire"], capture_output=True, timeout=5)
            time.sleep(1)
        except Exception as e:
...
def restart_audio_services() -> bool:
    ...
    try:
        ...
        try:
            ...
            subprocess.run(["jack_control", "start"], capture_output=True, timeout=10)
            time.sleep(2)
            success = True
...
def list_devices():
    ...
    for device in devices:
        input_str = (
            f"IN:{device['max_input_channels']}" if device["max_input_channels"] > 0 else "IN:0"
        )
        output_str = (
            f"OUT:{device['max_output_channels']}" if device["max_output_channels"] > 0 else "OUT:0"
        )
        print(
            f"[{device['index']:2d}] {device['name']} ({input_str}, {output_str}) @ {device['default_sample_rate']}Hz"
        )
...
def test_icusbaudio7d_device():
    ...
    for device in icusb_devices:
        ...
        if output_ok:
            ...
            print(
                f"  ✗ Only {output_channels} output channels detected (need {expected_outputs} for 5.1)"
            )
...
def test_respeaker_device():
    ...
    respeaker_devices = [
        ...
        or "array" in d["name"].lower()
        or "seeed" in d["name"].lower()
    ]
...
def reset_icusbaudio7d():
    ...
    if success:
        ...
        print(
            "The device should now show proper 5.1 surround sound capabilities (6+ output channels)."
        )
        ...
        print(
            "2. Check USB power management: sudo sh -c 'echo on > /sys/bus/usb/devices/*/power/control'"
        )
    ...
    for device in devices:
        ...
        if "icusbaudio7d" in name_lower:
            ...
            if device["max_input_channels"] > 0 and device["max_output_channels"] > 0:
                print(
                    f"  ✓ I/O channels: {device['max_input_channels']} in, {device['max_output_channels']} out"
                )
            else:
                print(
                    f"  ⚠️  I/O channels: {device['max_input_channels']} in, {device['max_output_channels']} out"
                )
    ...
    try:
        ...
        for source in respeaker_sources:
            try:
                ...
                subprocess.run(
                    ["pactl", "set-source-volume", source, f"{volume_percent}%"],
                    timeout=5,
                    check=True,
                )
...
def test_alsa_respeaker_access():
    ...
    try:
        ...
        result = subprocess.run(
            ["arecord", "-D", "respeaker", "--dump-hw-params"],
            capture_output=True,
            text=True,
            timeout=5,
        )
...
def fix_jack_shared_memory():
    ...
    try:
        ...
        try:
            subprocess.run(["jack_control", "start"], capture_output=True, timeout=15)
...
def test_jack_audio_playback(test_file=None):
    ...
    try:
        ...
        try:
            ...
            result = subprocess.run(
                ["timeout", "10s", "ffplay", "-nodisp", "-autoexit", "-f", "jack", test_file],
                capture_output=True,
                text=True,
                timeout=15,
            )
        ...
        try:
            ...
            result = subprocess.run(
                ["timeout", "5s", "aplay", "-D", "hw:3,0", test_file],
                capture_output=True,
                text=True,
                timeout=10,
            )
    ...
    try:
        if args.action == "list":
            ...
            if jack_ok:
                ...
                print(
                    "⚠️  JACK system not ready. Try running: uv run scripts/audio_recovery.py restart-services"
                )
