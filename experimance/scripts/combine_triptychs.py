...
The script processes images in groups:
    - Preview (2240x424) - panorama preview, used to identify groups
    - Left tile (1344x760)
    - Middle tile (1480x760) - has overlap on left side
    - Right tile (1480x760) - has overlap on left side

The tiles are combined with linear blending in the overlap regions.

Usage examples:
    # Combine all triptychs
    uv run scripts/combine_triptychs.py /path/to/images -o /path/to/combined

    # Then create video with existing script
    uv run scripts/images_to_video.py /path/to/combined -o output.mp4

    # Filter by time range
    uv run scripts/combine_triptychs.py /path/to/images -o /path/to/combined \\
        --start-time "2025-09-10 14:00:00" --end-time "2025-09-10 18:00:00"

    # Preview mode - just list groups
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=90,
        help="Quality for jpg/webp (1-100, default: 90)"
    )
    ...
    parser.add_argument(
        "--start-time",
        type=str,
        help="Filter: only include images after this time"
    )
    
    parser.add_argument(
        "--end-time",
        type=str,
        help="Filter: only include images before this time"
    )
    
    parser.add_argument(
        "--list-only",
        action="store_true",
        help="Just list triptych groups without combining"
    )
