...
Calibrates per-channel delays for multi-speaker setups so echo cancellation
works correctly on a conference mic/speaker device (e.g. Yealink).

Requires a PipeWire virtual multi-channel sink created by pipewire_multi_sink.py.
...
See docs/test_multi_channel_audio.md for full setup guide, calibration workflow,
interactive commands, and Linux audio troubleshooting.
"""
...
class ClickTrackGenerator:
    ...
    def generate_click_track(
        self, bpm: int = 120, duration_seconds: int = 10, click_frequency: int = 1000
    ) -> np.ndarray:
...
class VoiceAudioLoader:
    ...
    def load_wav_file(filepath: str) -> Tuple[np.ndarray, int]:
        ...
        with wave.open(filepath, "rb") as wav_file:
            ...
            logger.info(
                f"  Sample rate: {sample_rate}Hz, Channels: {num_channels}, Duration: {num_frames / sample_rate:.2f}s"
            )
...
class MultiChannelAudioTester:
        ...
        try:
            ...
            if device_name:
                ...
                if device_idx is not None:
                    ...
                    logger.info(
                        f"Using output device from config: {device_name} (index {device_idx})"
                    )
    ...
    def initialize_audio(self, device_name: str = "default"):
        ...
        try:
            with suppress_audio_errors():
                ...
                logger.info(
                    f"Initialized audio output: {self.channels} channels @ {self.sample_rate}Hz"
                )
    ...
    def _initialize_delay_buffers(self):
        ...
        self.delay_buffers = create_delay_buffers(
            self.channel_delays, self.sample_rate, self.max_delay_seconds
        )
    ...
    def _update_delay_buffer(self, channel: int, delay_seconds: float):
        ...
        logger.info(
            f"Updated delay buffer for channel {channel}: "
    ...
    def initialize_input(self, input_device_name: Optional[str] = None):
        ...
        try:
            if input_device_name:
                self.input_device_index = find_audio_device_by_name(
                    input_device_name, input_device=True
                )
    ...
    def interactive_delay_adjustment(self):
        ...
        print(
            f"Current delays (ms): {', '.join(f'Ch{ch}: {delay * 1000:.1f}' for ch, delay in self.channel_delays.items())}"
        )
        print("\nCommands:")
        ...
        while True:
            try:
                ...
                if cmd[0] == "q":
                    ...
                    if len(cmd) > 1:
                        try:
                            ...
                            self.selected_channels = [
                                ch for ch in self.selected_channels if 0 <= ch < self.channels
                            ]
                            print(
                                f"Selected channels for interactive mode: {self.selected_channels}"
                            )
                        except ValueError:
                    ...
                    print("       play voice - Play loaded voice audio")
                    print(
                        "       i [channels] - Interactive mode (e.g., 'i 0,1' for channels 0 and 1)"
                    )
        ...
        try:
            ...
            while True:
                ...
                if now - last_update > 0.5:
                    last_update = now
                    all_delays_str = ", ".join(
                        f"Ch{ch}:{self.channel_delays.get(ch, 0.0) * 1000:.1f}ms"
                        for ch in range(self.channels)
                    )
    ...
    def _analyze_recording(self):
        ...
        peaks, _ = signal.find_peaks(
            np.abs(filtered), height=np.max(np.abs(filtered)) * 0.3, distance=self.sample_rate // 4
        )
...
def main():
    ...
    parser.add_argument(
        "--file",
        "-f",
        help="Audio file to load for testing (e.g., media/audio/cartesia_sophie.wav)",
    )
    parser.add_argument(
        "--auto-calibrate",
        "-a",
    ...
    parser.add_argument(
        "--list-devices", "-l", action="store_true", help="List audio devices and exit"
    )
    parser.add_argument(
