...
class TestTaskManangement:
    """Tests for task management in BaseService."""
    
    @pytest.mark.asyncio
    async def test_add_task(self):
        """Test adding a task to the service."""
        service = BaseService("task-add-test-service", "test")
        ...
        assert len(service.tasks) == 1, "Task should be added to the service"
    
    @pytest.mark.asyncio
    async def test_clear_tasks(self):
        """Test clearing all tasks from the service."""
        service = BaseService("task-clear-test-service", "test")
        ...
        assert len(service.tasks) == 0, "All tasks should be cleared from the service"

    @pytest.mark.asyncio
    async def test_add_task_accepts_coroutine_and_creates_task(self):
        """Test that add_task accepts a coroutine and schedules it 
        to run on run() or immediately if service running."""
        service = BaseService(service_name="add-task-test-service")
        ...
        # Pass coroutine directly, not a Task
        service.add_task(sample_task())
    ...
    @pytest.mark.asyncio
    async def test_add_task_accepts_coroutine_and_runs_task(self):
        """Test that add_task accepts a coroutine and schedules it 
        to immediately if service running."""
        service = BaseService(service_name="add-task-test-service")
